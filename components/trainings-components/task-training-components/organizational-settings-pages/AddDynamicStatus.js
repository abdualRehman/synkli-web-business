...
const AddDynamicStatus = ({ toggleAddStatus, toggleStatusUpdate }) => {
  ...
  const [className, setClasssName] = useState("task-add-field-indicator1");
  ...
  const handleAddInput = (e, index) => {
    ...
    if (e.target.value.length > 9) {
      if (fields.length == 3) {
        setClasssName("submit-indicator");
        return;
      }
      setClasssName(`add-button-indicator${index + 1}`);
    }
    setFields(newValues);
  };
  ...
  const handleAddField = (index) => {
    ...
    setFields(newValues);
    setClasssName(`task-add-field-indicator${index + 2}`);
  };
  ...
  useEffect(() => {
    setTimeout(() => {
      driverObjs.highlight({
        element: `.${className}`,
        popover: {
          description: className.includes("task-add")
            ? "Please enter title in highlighted field atleast 10 charactors"
            : className.includes("submit-indicator")
            ? "Click here to add"
            ...
            <div className="add-status-container">
                    ...
                    <div className="grid grid-cols-12 mt-1">
                      <div
                        className={`col-span-11 
                          task-add-field-indicator${index + 1}
                        }`}
                      >
                        <input
                          disabled={
                            className.includes(
                              `button-indicator${index + 1}`
                            ) && true
                          }
              ...
              <div className="absolute bottom-5 w-full flex justify-center items-center bg-white py-2">
                <button
                  ...
                  type="submit"
                  className="add-btn text-white px-5 md:px-28 py-2 flex gap-2 items-center cursor-pointer  rounded-lg submit-indicator "
                >
                  <span>Add</span>
