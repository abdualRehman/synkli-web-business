export const TimesheetJumbo = ({ toggleAddTimesheet }) => {
  return (
      ...
      <div className="profile-jumbo-flex">
        <div className="jumbo-flex-1 flex flex-col w-full relative">
          ...
          <div className="jumbo-dir md:mt-2">
            Payroll <span className="special-jumbo-text">&gt; timesheet</span>
          </div>

          <div className="md:absolute right-5 top-5  flex gap-2">
            <button
              onClick={toggleAddTimesheet}
              className="invite-people-btn cursor-pointer"
            >
              Add Timesheet
            </button>
            <button className="leave-schedule-btn">Beta-Feedbacks</button>
          </div>
        </div>
      </div>
    </div>
